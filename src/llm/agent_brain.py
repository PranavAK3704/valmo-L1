...
class TicketUnderstanding:
    """Output of Stage 1+2 — comprehension + scenario matching."""
    clean_problem: str = ""
    ops_translation: str = ""
    key_signals: List[str] = field(default_factory=list)
    is_gibberish: bool = False
    gibberish_reason: str = ""
    scenario_matches: List[Dict] = field(default_factory=list)  # top 3 from catalog
...
class AgentBrain:
    ...
    def understand(self, ticket: Dict) -> TicketUnderstanding:
        """
        Stage 1 + 2: Comprehension + Scenario Matching.
        Translates raw ticket → clean ops language → top 3 scenario matches.
        Does NOT resolve. Call resolve_with_scenario() after user approves.
        """
        from src.llm.comprehension import understand_ticket, match_scenario

        logger.info(f"[Brain.understand] ticket={ticket.get('ticket_id')}")

        # Stage 1: comprehension
        stage1 = understand_ticket(ticket)
        clean_problem = stage1.get("clean_problem", "")
        key_signals = stage1.get("key_signals", [])

        # Stage 2: scenario matching
        queue = ticket.get("queue") or ticket.get("queue_key") or ""
        matches = match_scenario(clean_problem, key_signals, queue)

        return TicketUnderstanding(
            clean_problem=clean_problem,
            ops_translation=stage1.get("ops_translation", ""),
            key_signals=key_signals,
            is_gibberish=stage1.get("is_gibberish", False),
            gibberish_reason=stage1.get("gibberish_reason") or "",
            scenario_matches=matches,
        )

    def resolve_with_scenario(self, ticket: Dict, scenario_id: str,
                               query_results: List[Dict]) -> BrainDecision:
        """
        Stage 3: Resolve using the approved scenario.
        Pulls the exact SOP text for the scenario and calls Gemini once.
        """
        from src.llm.scenario_catalog import get_scenario

        scenario = get_scenario(scenario_id)
        logger.info(f"[Brain.resolve] ticket={ticket.get('ticket_id')} scenario={scenario_id}")
        ...
        queue = ticket.get("queue") or ticket.get("queue_key") or ""
        steps.append({
            "icon": "🔍", "label": "Reading ticket",
            "detail": f"Queue: {queue} | AWBs: {', '.join(awbs) if awbs else 'None'} | Scenario: {scenario.get('title', scenario_id)}",
            "status": "done",
        })

        # Get SOP text for this specific scenario
        sop_query = f"{scenario.get('title', '')} {scenario.get('sop_reference', '')} {' '.join(scenario.get('signals', []))}"
        sop_text = self._sop.retrieve(sop_query, k=4)
        steps.append({
            "icon": "📖", "label": f"SOP loaded: {scenario.get('sop_reference', scenario_id)}",
            "detail": f"Exact SOP for approved scenario — {scenario.get('action_detail', '')[:100]}",
            "status": "done",
        })
        ...
        if query_results:
            for qr in query_results:
                rows = (qr.get("data") or {}).get("rows", [])
                name = qr.get("query_name", "query").replace("_", " ")
                status = "done" if qr.get("success") else "warning"
                detail = f"{len(rows)} row(s) returned" if qr.get("success") else f"Failed — {qr.get('error', '')}"
                steps.append({"icon": "📊", "label": f"Metabase: {name}", "detail": detail, "status": status})
        else:
            steps.append({"icon": "📊", "label": "Metabase queries", "detail": "No AWBs — skipped", "status": "warning"})

        # Build resolution prompt
        clean_problem = ticket.get("clean_problem", "")
        query_text = _format_query_results(query_results)
        attachment_summary = _extract_attachment_summary(ticket)
...
## CAPTAIN'S ORIGINAL MESSAGE
Subject: {ticket.get('subject', '')}
Detail: {ticket.get('detail') or ticket.get('full_description') or ticket.get('description') or ''}

## CLEAN PROBLEM (STAGE 1 TRANSLATION)
{clean_problem or ticket.get('subject', '')}

## APPROVED SOP SCENARIO
ID: {scenario_id}
Title: {scenario.get('title', '')}
Expected Action: {scenario.get('action_detail', '')}
SOP Reference: {scenario.get('sop_reference', '')}

## EXACT SOP TEXT FOR THIS SCENARIO
{sop_text}
...
---
You have been given the EXACT matched scenario. Follow the SOP precisely.
Respond with the standard JSON decision format.
"""
        steps.append({"icon": "🤔", "label": "Reasoning with Gemini...",
                      "detail": f"Applying {scenario.get('sop_reference', scenario_id)} rules",
                      "status": "thinking"})
        ...
        decision = BrainDecision(
            ...
            missing_fields=raw.get("missing_fields", []),
            confidence=float(raw.get("confidence", 0)),
            scenario_identified=raw.get("scenario_identified", scenario.get("title", scenario_id)),
            reasoning=raw.get("reasoning", ""),
            usage=raw.get("_usage", {}),
        )
        decision.auto_send = decision.action == "respond" and decision.confidence >= AUTO_SEND_CONFIDENCE

        steps[-1] = {
            "icon": "🤔", "label": "Gemini reasoning complete",
            "detail": (decision.reasoning[:200] + "…") if len(decision.reasoning) > 200 else decision.reasoning,
            "status": "done",
        }
        ...
        if decision.action == "respond":
            auto_tag = " · AUTO-SEND" if decision.auto_send else " · Queued for review"
            steps.append({"icon": "✅", "label": f"Decision: RESPOND — confidence {decision.confidence}/10",
                          "detail": (decision.scenario_identified or scenario.get("title")) + auto_tag, "status": "done"})
        elif decision.action == "escalate":
            steps.append({"icon": "🔺", "label": f"Decision: ESCALATE — confidence {decision.confidence}/10",
                          "detail": f"→ {decision.escalation_queue or 'L2'} | {decision.escalation_reason[:100]}",
                          "status": "done"})
        else:
            upgrade_msg = _blocker_upgrade(ticket, decision, query_results)
            steps.append({"icon": "🚫", "label": f"Blocked — {decision.action}",
                          "detail": decision.stuck_question or "Missing data to resolve",
                          "status": "blocked", "upgrade": upgrade_msg})

        decision.thinking_steps = steps
        logger.info(f"[Brain.resolve] action={decision.action} confidence={decision.confidence}")
        return decision
